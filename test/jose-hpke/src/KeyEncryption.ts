...
const produceDecryptionResult = async (protectedHeader: string, ciphertext: string, tag: string, iv: string, cek: Uint8Array, aad?: string) => {
  ...
  if (aad) {
    const additionalAuthenticatedData = new Uint8Array(base64url.decode(aad))
    decryption.additionalAuthenticatedData = additionalAuthenticatedData;
  }
  return decryption
}
...
export const decrypt = async (req: RequestGeneralDecrypt, options = { serialization: 'GeneralJson' }): Promise<any> => {
    ...
    recipients = [
      {
        encrypted_key,
        header: {
          kid: req.recipients.keys[0].kid,
          alg: req.recipients.keys[0].alg,
  ...
  let matchingRecipient = undefined as any
  let matchingPrivateKey = undefined as any
  for (const privateKey of req.recipients.keys) {
    ...
    if (recipient) {
      ...
      matchingPrivateKey = privateKey as any;
      break
    }
  }
  ...
  if (isKeyAlgorithmSupported(matchingPrivateKey)) {
    ...
    const backupAlg = matchingPrivateKey.alg
    delete matchingPrivateKey.alg
    ...
    matchingPrivateKey.alg = backupAlg
